...
/**
 * @author:小春
 * @date:2015年5月20日上午12:06:54
 * @version
 */

public class PlayerInMatchVO {
   ...
   String FTPer;//罚球命中率
	...
	double PLUS_MINUS;//真實命中率
	 ...
	 public PlayerInMatchVO(String gameID, String matchDate,
...
//			int tPA, int tPM, String tPPer, int fTA, int fTM, String fTPer,
//			int rEB, int oREB, int dREB, int aST, int sTL, int bLK, int tOV,
//			int pF, int pTS, String realHitPer) {
//		super();
//		this.gameID = gameID;
//		this.matchDate = matchDate;
//		this.typeOfMatch = typeOfMatch;
...
//		PF = pF;
//		PTS = pTS;
//		this.realHitPer = realHitPer;
//	}

	

	public PlayerInMatchVO(PlayerInMatchPO po) {
		super();
		...
		this.teamOfPlayer = po.getTeamOfPlayer();
		this.position = po.getPosition();
		this.isGS = po.getIsGS();
		MIN = po.getMIN();
		FGA = po.getFGA();
		FGM = po.getFGM();
		FGPer = po.getFGPer();
		...
		PF = po.getPF();
		PTS = po.getPTS();
		PLUS_MINUS = po.getPLUS_MINUS();
	}

	public double getNum(TypeOfSort4HotPointPlayer type) {
		String temp;
		double num;
		switch (type) {
		case SCORING:// 得分
		...
		case BLOCK:// 蓋帽
			return BLK;
		case STLSv_STEALS:// 抢断
			return STL;
		case TPPer:// 三分命中率
			temp = TPPer.replaceAll("%", "");
			num = Double.parseDouble(temp) / (double) 100;
			return num;
		case FGPer:// 投篮命中率
